...
select ok(basejump.get_config() ->> 'billing_provider' = 'stripe', 'Basejump config should have stripe as the billing provider');
select ok((basejump.get_config() ->> 'stripe_default_trial_period_days')::int = 30), 'Basejump config should have a default trial period';


select function_returns('basejump', 'generate_token', Array['integer'], 'bytea', 'Basejump generate_token function should exist');
select function_returns('basejump', 'trigger_set_timestamps', 'trigger', 'Basejump trigger_set_timestamps function should exist');
...
select ok(public.get_service_role_config() is not null), 'Basejump get_service_role_config should be accessible to the service role';
...
select ok(basejump.is_set('enable_personal_accounts')), 'Basejump is_set should be accessible to authenticated users';
select ok(basejump.generate_token(1) is not null), 'Basejump generate_token should be accessible to authenticated users';
select throws_ok('select public.get_service_role_config()');
