...
const createOrRetrieveSubscription = async ({
  ...
  try {
    ...
    if (!config.default_account_plan_id) {
      return;
    }

    const price = await stripe.prices.retrieve(config.default_account_plan_id);

    if (
      !price ||
      (price.unit_amount > 0 && !config["default_trial_period_days"])
    ) {
      return;
    }
    ...
    const newSubscription = await stripe.subscriptions.create({
      customer: customerId,
      items: [{ price: config["default_account_plan_id"] }],
      expand: ["latest_invoice.payment_intent"],
      trial_period_days: config["default_trial_period_days"],
    });
